...
/** Standard VDP VRAM layout for SCREEN 2 */
export const VDP_LAYOUT = {
...
/**
 * Captures writes to VDP ports and maintains a 16KB VRAM buffer.
 *
 * VDP register write protocol (port 0x99):
 *   1st write: data byte (value)
 *   2nd write: register number | 0x80
 */
export class VdpCapture {
  ...
  /** Handle a write to port 0x99 (VDP register port) */
  ...
  /** Handle a write to port 0x98 (VDP data port) */
  ...
  /** Handle a read from port 0x98 (VDP data port) */
  readData(): number {
  ...
  /** Handle a read from port 0x99 (VDP status port) */
  readStatus(): number {
    this.pendingByte = null
    return 0
  }
  ...
  readSatEntry(index: number): SatEntry {
    const base = VDP_LAYOUT.SAT + index * 4
    return {
      y:       this.vram[base],
      x:       this.vram[base + 1],
  ...
  readPntTile(col: number, row: number): number {
    return this.vram[VDP_LAYOUT.PNT + row * 32 + col]
  }
