 ...
 * Each entry maps a name to Z80 machine code placed sequentially.
 * Returns the machine and a map of name → address.
 */
function createMachineWithFunctions(
  funcs: Record<string, Uint8Array>,
): { m: Z80TestMachine; addr: Record<string, number> } {
  ...
  for (const [name, code] of Object.entries(funcs)) {
    addresses[name] = a
    regions.push([a, code])
    a += code.length + 4 // gap between functions
  }

  return {
    m: new Z80TestMachine({ regions }),
    addr: addresses,
  }
}
...
describe('callC', () => {
  ...
  describe('stack arguments', () => {
    it('second u8 arg goes to stack', () => {
      // The function reads SP+2 (skip return addr pushed by runFrom)
...
describe('defC', () => {
  ...
  describe('reusable signatures', () => {
    it('same signature can bind to different machines', () => {
      // Both machines place 'inc' at the same address (CODE_BASE)
      const { m: m1, addr: addr1 } = createMachineWithFunctions({
        inc: new Uint8Array([0x3C, 0xC9]),
      })
      const { m: m2 } = createMachineWithFunctions({
